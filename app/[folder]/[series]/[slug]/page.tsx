...
export default async function SeriesPostPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 pt-6 pb-12">
      <Link href={`/${blog.folder}/series`} className="inline-flex items-center text-sm font-medium text-foreground/70 hover:text-primary transition-colors mb-8 group">
        <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-1 transition-transform" /> Back to Series
      </Link>
      ...
      <article className="space-y-8">
        ...
        <header className="space-y-4">
          <div className="mb-2">
            <span className="text-xs font-bold text-primary uppercase tracking-wider">SERIES: {series}</span>
          </div>
